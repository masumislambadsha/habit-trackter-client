...
import type { ApiError, Habit } from "../types/app";
...
const MyHabits = () => {
  ...
  const handleComplete = async (id: string, habitTitle: string) => {
    ...
    try {
      ...
      const error = err as ApiError;
      if (
        error.response?.data?.error === "Already completed today" ||
        ...
        toast.error(error.response?.data?.error || "Failed to mark complete");
      }
    } finally {
